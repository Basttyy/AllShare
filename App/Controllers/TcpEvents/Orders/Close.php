...
final class Close
{
    ...
    public function __invoke (WebSocket $conn, string $data)
    {
        try {
            ...
            } else if (\strpos($data["message"], "unable to close order with ticket ") !== false) {

            } else if (\strpos($data["message"], "closed order with ticket") !== false) {
            ...
            //$conn->send("events/order/close{$separator}{$message}");
        } catch (Exception $ex) {
            echo $ex->getMessage();
        }
    }
